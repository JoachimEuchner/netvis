...
ActionListener
{
   ...
   */
   ...
   public void paintOS(Graphics g)
   {
     ...
     String s = "g2.paint(): " + (paintend - paintstart) + " ms/"+paintCounter+" "+mModel.getAllNodes().size()+"nodes"; //$NON-NLS-1$ //$NON-NLS-2$
   ...
   private class MyLayouter implements Runnable
   {
     ...
     public void run()
     {
       ...
       logger.trace("MyLayouter.run() start layouting.");
       ...
       while ( mHost.doLayouting )
       {
         ...
         logger.trace("MyLayouter.run() layout: {}ms, {}nodes", (layoutend - layoutstart), mModel.getAllNodes().size());
